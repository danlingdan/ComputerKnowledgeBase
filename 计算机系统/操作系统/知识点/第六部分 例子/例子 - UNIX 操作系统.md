...
| 操作 | 关联的 UNIX 抽象 | 对应知识 |
| --- | --- | --- |
| 解析路径 | 目录、挂载点、符号链接 | [[第十章 文件系统#10.3 目录与磁盘的结构]] |
| 打开文件 | 文件描述符、打开文件表 | [[第十章 文件系统#10.1 文件概念]] |
| 读写数据 | 页缓存、逻辑块、文件偏移 | [[第十一章 文件系统实现]] |
| 写回介质 | 缓冲、设备队列、驱动 | [[第十三章 IO系统]] |
| 分配空间 | 块映射、空闲空间与恢复 | [[第十一章 文件系统实现#11.4 分配方法]] |

编译器读取源文件、头文件和库文件，在用户地址空间中解析它们；输出的目标文件或可执行文件经文件系统写入存储。文件格式由编译器、链接器和加载器约定，普通文件系统通常只将其视为字节序列。

### 虚拟内存与调页

编辑器和编译器看到的是各自独立的虚拟地址空间。代码段、堆、栈和映射文件按页组织；缺页时内核从后备存储取回页面或分配零页。内存不足时，页面可能被换出到交换空间，具体机制见 [[第九章 虚拟内存管理]] 与 [[第十二章 大容量存储设备#12.6 交换空间管理]]。
...
若两个进程同时修改同一个文件，文件系统的写入成功不等于应用语义正确。临界区、互斥、死锁与文件锁分别见 [[第六章 同步]]、[[第七章 死锁]] 和 [[第十章 文件系统#10.5 文件共享]]。
